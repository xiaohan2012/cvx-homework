...
cvx_begin
    ...
    variables s1(M) s2(N) s3(P)
    minimize lambda1 * (vone1*s1 + vone2*s2 + vone3*s3) + norm(a1, 2) + norm(a2, 2) + norm(a3, 2)
    subject to
        X'*a1-b1 >= max(X'*a2-b2, X'*a3-b3) + 1 - s1; 
        Y'*a2-b2 >= max(Y'*a1-b1, Y'*a3-b3) + 1 - s2;
        Z'*a3-b3 >= max(Z'*a1-b1, Z'*a2-b2) + 1 - s3;
        % a1 + a2 + a3 == 0;
        % b1 + b2 + b3 == 0;
	s1 >= 0;
	s2 >= 0;
